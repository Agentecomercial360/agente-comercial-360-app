...
> = {
  ...
  success_empty: {
    label:
      "Sincronizada sem vendas no período",
    badge:
...
export function AccountsSyncStatus({
  ...
  async function load() {
    ...
    try {
      const [accountResult, runResult] =
        await Promise.all([
          supabase
            ...
            .select(
              "id, account_name, nickname, marketplace, is_active",
            )
            .eq(
              "company_id",
              companyId,
      ...
      setAccounts(
        (
          accountResult.data as unknown as AccountRow[]
        ) ?? [],
      );

      setRuns(
        (
          runResult.data as unknown as SyncRun[]
        ) ?? [],
      );
    } catch (caughtError) {
      setError(
        caughtError instanceof Error
          ? caughtError.message
          : "Erro ao carregar status das contas",
      );
    } finally {
      setLoading(false);
    }
          ...
          return (
            ...
            >
              ...
              <dl className="mt-3 grid grid-cols-2 gap-x-3 gap-y-1.5 text-[11px]">
                ...
                <Field
                  label="Pendentes de custo"
                  value={
                    run
                      ? num.format(
                          run.orders_pending_cost ??
                            0,
                        )
                      : "—"
                  }
