...
function StatusMessage({
  ...
  const previewError =
    error instanceof StudioIaPreviewError
      ? error
      : null;
...
export function StudioIaSourcesStatus() {
  ...
  const previewQuery =
    useQuery<
      StudioIaContextPreview,
      Error
    >({
      queryKey: [
        "studio-ia-context-preview",
        companyId,
        activeAccountId,
      ],
  ...
  const hasUnconfirmedSource =
    previewQuery.data?.sources.some(
      (
