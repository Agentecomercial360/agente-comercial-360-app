            ...
            >
              <div className="flex flex-wrap items-start justify-between gap-2">
                ...
                <div className="flex flex-wrap items-center gap-1.5">
                  {item.severity && (
                    <span
                      className={`rounded-md border px-2 py-0.5 text-[11px] font-semibold ${severityClass(
                        item.severity,
                      )}`}
                    >
                      {item.severity}
...
export function StudioIaDiagnosticSection() {
  ...
  const costsPending =
    ...
    (
      data?.financialStatus.cost_status ?? ""
    ).toLowerCase() === "pending_real_costs";
            ...
            <>
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <MetricCard
                  ...
                  value={
                    data.summary.costs_pending === null
                      ? "—"
                      : costsPending
                        ? "Sim"
                        : "Não"
                ...
                <MetricCard
                  ...
                  value={`${fmtInt(
                    data.summary.sources_available,
                  )} / ${fmtInt(
                    data.summary.sources_checked,
                  )}`}
                />
              ...
              <p className="mt-4 rounded-xl border border-blue-100 bg-blue-50/60 px-4 py-3 text-xs text-blue-800">
                As recomendações são uma prévia operacional. Qualquer ação futura deverá ser aprovada por um humano.
              </p>
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-3">
              <MetricCard
                label="Receita"
                value={
                  data.financialStatus.revenue_status ??
                  "—"
                }
              />
            ...
            {costsPending && (
              <div className="mt-3 flex items-start gap-2 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">
                ...
                <span>
                  <strong>
                    Custos reais pendentes.
                  </strong>{" "}
                  A análise de margem e lucro será liberada após a importação dos custos pelos operadores.
                </span>
              </div>
            )}
          </section>
